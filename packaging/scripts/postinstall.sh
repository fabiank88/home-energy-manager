...
USER_CHOICE_CONFIG="/etc/hem-userchoices.sh"
ETC_SERVICE="/etc/systemd/system/hem.service"
USR_LOCAL_BIN="/usr/local/bin/hem"
...
if [ "$1" = "configure" ]; then
	...
	if [ -f "$ETC_SERVICE" ] && [ "$KEEP_ETC_SERVICE" -eq 0 ]; then
		...
		echo "You probably want to delete this file now. Your hem configuration stays untouched!"
		askUserKeepFile "$ETC_SERVICE" || KEEP_ETC_SERVICE=$?
	fi
	if [ -f "$USR_LOCAL_BIN" ] && [ "$KEEP_USR_LOCAL_BIN" -eq 0 ]; then
		echo "An alternate hem binary was detected under '$USR_LOCAL_BIN'."
		echo "This is probably due to a previous manual installation."
		echo "You probably want to delete this file now. Your hem configuration stays untouched!"
...
EOF
	...
	if [ -f "$USR_LOCAL_BIN" ] && [ "$KEEP_USR_LOCAL_BIN" -eq 0 ]; then
		echo "Deleting old hem binary '$USR_LOCAL_BIN'"
		rm -v "$USR_LOCAL_BIN"
	fi
fi
...
if [ "$1" = "configure" ] || [ "$1" = "abort-upgrade" ] || [ "$1" = "abort-deconfigure" ] || [ "$1" = "abort-remove" ] ; then
	...
	deb-systemd-helper unmask hem.service >/dev/null || true

	# was-enabled defaults to true, so new installations run enable.
	if deb-systemd-helper --quiet was-enabled hem.service; then
		# Enables the unit on first installation, creates new
		# symlinks on upgrades if the unit file has changed.
		deb-systemd-helper enable hem.service >/dev/null || true
	else
		# Update the statefile to add new symlinks (if any), which need to be
		# cleaned up on purge. Also remove old symlinks.
		deb-systemd-helper update-state hem.service >/dev/null || true
	fi
	...
	if [ -d /run/systemd/system ]; then
		...
		if [ -f $RESTART_FLAG_FILE ]; then
			deb-systemd-invoke start hem.service >/dev/null || true
			rm $RESTART_FLAG_FILE
		elif [ -n "$2" ]; then
			deb-systemd-invoke try-restart hem.service >/dev/null || true
		else
			deb-systemd-invoke start hem.service >/dev/null || true
		fi
	fi
